...
function main(inputCity) {
  // основная(начальная) функция
  let result = allCity.some((elem) => elem == city);
  result ? deleteCity(city) : inputErrors();
...
function deleteCity(nameCity) {
  // инспектор done input
...
function inputErrors(city) {
  // инспектор error input
  if (variable == 0)
    main(
      prompt(
        `Не успели начать, а уже ошибка. Введите имя города корректно пожалуйста!`
      )
    );
  else {
...
function checkCorrectInputLetter(inputCityName) {
  // глобальная проверка введенного города
  ...
  if (firstLetterInputCityName != lastLetterInArrLetter) {
    checkCorrectInputLetter(
      prompt(
        `Вы написали город не на ту букву. Вам на: ${lastLetterInArrLetter}.`
      )
    );
  } else {
    if (getUsCity == true) {
      // проверка на наличие введенного города в массиве ИСПОЛЬЗОВАННЫХ городов
      checkCorrectInputLetter(
        prompt(
          `Город уже называли. давайте снова. Вам на: ${lastLetterInArrLetter}.`
        )
      );
    } else {
      if (getAllCity) {
        // проверка на наличие введенного города в массиве ВСЕХ городов
        main(inputCityName);
      } else {
        checkCorrectInputLetter(
          prompt(`Введите город правильно. Вам на: ${lastLetterInArrLetter}.`)
        );
      }
    }
  }
...
function getCityOnLustLetter(lustLatter) {
  // выдать игроку город на посл-ю бук-у его города
  for (let i = 0; i < allCity.length; i++) {
    ...
    if (firstLetterCity == lustLatter) {
      ...
      checkCorrectInputLetter(
        prompt(
          `${cityOnThisLatter}, вам на: ${
            arrLustLatter[arrLustLatter.length - 1]
          }`
        )
      );
    }
...
function getLastLetterCity(elem) {
  // определение последней буквы города
